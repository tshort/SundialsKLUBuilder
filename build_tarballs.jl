...
# Collection of sources required to build SundialsKLUBuilder
sources = [
...
mkdir build
cd build
cmake -DCMAKE_INSTALL_PREFIX=/ -DCMAKE_TOOLCHAIN_FILE=/opt/$target/$target.toolchain -DEXAMPLES_ENABLE=OFF -DKLU_ENABLE=ON DKLU_INCLUDE_DIR=$WORKSPACE/srcdir/SuiteSparse/KLU/Include/ -DKLU_LIBRARY_DIR=$WORKSPACE/srcdir/SuiteSparse/lib ..
make -j8
make install
...
hashes = autobuild(pwd(), "SundialsKLUBuilder", platforms, sources, script, products)

if !isempty(get(ENV,"TRAVIS_TAG",""))
    print_buildjl(pwd(), products, hashes,
        "https://github.com/LibGit2.GitRepo("/tmp/juliamjdpyu")/releases/download/$(ENV["TRAVIS_TAG"])")
end
